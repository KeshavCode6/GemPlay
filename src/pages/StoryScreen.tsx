...
export default function StoryScreen() {
    ...
    const [currentNode, setCurrentNode] = useState<{ topic: string; paths?: any[] } | null>(null);
    ...
    useEffect(() => { console.log(currentNode) }, [currentNode])
            ...
            >
                <AnimatePresence mode="wait">
                    ...
                    >
                                ...
                                {currentNode ? (
                                    <div className="grid grid-cols-1 gap-1 mt-8">
                                        {currentNode.paths?.length !== 0 && currentNode.paths ? (
                                            currentNode.paths?.map((option: { topic: string, paths: any }, index: number) => (
                                                <Button className="w-full h-[8rem] text-lg text-white" key={index} onClick={() => { activeOption(index) }}>
                        ...
                        ) : <Story currentNode={currentNode} nextTopic={() => { setActive(false) }} />}
                    </motion.div>
                </AnimatePresence>
            </motion.div>
        </motion.div>
